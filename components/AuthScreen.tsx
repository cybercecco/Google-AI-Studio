  ...
  const handleLocalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If API returns explicit error (e.g. 401), throw to trigger fallback/error handling
        throw new Error("API Error");
      }
    } catch (err) {
      // FALLBACK MOCK AUTHENTICATION
      // This allows the app to work for UI testing even without the PHP backend
      console.warn("Backend unavailable, using local fallback auth.");
      ...
      if (email === 'admin' && password === '12345678') {
      ...
      } else if (email === 'user' && password === '12345678') {
        onLogin({
            id: 'local-user',
            name: 'Standard User',
        ...
        setError(lang === 'it' ? 'Credenziali non valide (Default: admin/12345678)' : 'Invalid credentials (Default: admin/12345678)');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      const mockUser: User = {
        ...
        role: 'user' // Default to normal user for SSO mocks unless specified
      };
      onLogin(mockUser);
      setIsLoading(false);
    }, 1500);
